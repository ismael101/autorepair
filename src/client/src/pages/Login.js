...
export default function Login(){
    ...
    const { token, isLoading, isError, isSuccess, error } = useSelector(
        (state) => state.auth
    )
        ...
        <div className="h-screen bg-gray-300 flex">
            <div className="rounded-md w-1/3 px-10 pt-10 h-1/2 mt-36 pb-2 bg-white mx-auto">
                <form className="space-y-5" onSubmit={handleSubmit}>
                    ...
                    <div>
                        <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 block w-full text-center mt-10">Submit</button>
                    </div>
                </form>
                <Link to="/signup" ><p className="mt-5 text-center text-md text-blue-700">Signup</p></Link>
            </div>
        </div>
    )
}