...
export default function Customer(props){
    ...
    const handleUpdate = () => {
        setEdit(false)
        dispatch(updateCustomer({id:props.customer.id, first:first, last:last, email:email, phone:phone}))
    }
                                ...
                                <div>
                                    <form onSubmit={() => handleUpdate()}>
                                        <div className="mx-4 mb-1 mt-5"> 
                                            ...
                                            <input value={first} onChange={(e) => {setFirst(e.target.value)}} type="text" id="first" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="John" required/>
                                        </div>
                                        <div className="mx-4 my-1">
                                            <label htmlFor="last" className="block mb-2 text-sm font-medium">Last</label>
                                            <input value={last} onChange={(e) => {setLast(e.target.value)}} type="text" id="last" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Smith" required/>
                                        </div>
                                        <div className="mx-4 my-1">
                                            <label htmlFor="email" className="block mb-2 text-sm font-medium">Email</label>
                                            <input value={email} onChange={(e) => {setEmail(e.target.value)}} type="email" id="email" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="johnsmith@gmail.com" required/>
                                        </div>
                                        <div className="mx-4 my-1">
                                            <label htmlFor="phone" className="block mb-2 text-sm font-medium">Phone</label>
                                            <input value={phone} onChange={(e) => {setPhone(e.target.value)}} type="text" id="phone" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="6122493211" minLength={10} maxLength={10} required/>
                                        </div>
